...
((method == VIQR_INPUT)? ID_VIQR_TYPING : \
((method == TELEX_MUONG) ? ID_TELEX_MUONG_TYPING : ID_VIQR_STAR_TYPING))))

#define cmdToInputMethod(cmd) \
((cmd == ID_TELEX_TYPING)? TELEX_INPUT : \
((cmd == ID_VNI_TYPING)? VNI_INPUT : \
((cmd == ID_VIQR_TYPING)? VIQR_INPUT : \
((cmd == ID_TELEX_MUONG_TYPING)? TELEX_MUONG : VIQR_STAR_INPUT))))
