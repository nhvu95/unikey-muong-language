...
AnsiDlgItemInfo VnDlgItems[] = { //use explicit ansi string for vietnamese in TCVN3
	{IDC_FREE_MARKING,		"Cho phep go dau mu, moc o cuoi tu"},
	{IDC_TONE_CTRL,			"Phai go dau ngay sau nguyen am"},
	{IDC_MODERN_STYLE,		"Su dung oa` uy'(thay vi o`a u'y)"},
	{IDC_VIET_GUI,			"Vietnamese interface"},
	{IDC_AUTO_SPELL,		"Tu dong sua loi chinh ta"},
	{IDC_SHOW_DLG,			"Bat hoi thoai nay khi khoi dong"},
	{IDC_RESET,				"Mac dinh"},
	{IDC_EXIT,				"Ket thuc"},
	{IDOK,					"Dong"},
	{IDC_OPEN_HELP,			"Huong dan"},
	{IDC_ABOUT,				"Thong tin"},
	{IDC_CONTROL_GROUP,		"Dieu Khien"},
	{IDC_CHARSET_PROMPT,	"Bang ma:"},
	{IDC_SWITCHKEY_PROMPT,	"Phim chuyen"},
	{IDC_KEYMODE_PROMPT,	"Kieu go:"},
	{IDC_OPTION_GROUP,		"Tuy chon khac"}, //"Lùa chän"
	{IDC_AUTO_START,		"Khoi dong cung Windows"},
	{IDC_MACRO_CHECK,		"Cho phep go tat"},
	{IDC_MACRO_BTN,			"Bang go tat"},
	{IDC_USE_CLIPBOARD,     "Su dung clipboard cho unicode"}
};

#define MAX_ITEM_LEN 61
...
DlgItemInfo EnDlgItems[] = { // use generic string
	...
	{IDC_AUTO_SPELL,		_T("Auto fix spell")},
...
void CKeyDlg::loadDlgInfo(CUserPref *pPref)
{
	...
	SendMessage(m_spellCheck,
		BM_SETCHECK,
		(pPref->m_spellAutoFix) ? BST_CHECKED : BST_UNCHECKED,
		0);
	...
	if (!MainWnd.isVietFontOK())
		EnableWindow(GetDlgItem(m_hWnd, IDC_VIET_GUI), TRUE);
...
void CKeyDlg::storeDlgInfo(CUserPref *pPref)
{
	...
	sel = (SendMessage(m_spellCheck, BM_GETCHECK, 0, 0) == BST_CHECKED) ? 1 : 0;
	if (sel && !pPref->m_spellAutoFix || !sel && pPref->m_spellAutoFix) {
		pPref->m_changes |= PREF_CHANGE_OPTIONS;
		pPref->m_spellAutoFix = sel;
	}
