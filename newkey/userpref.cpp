...
int CUserPref::setValue()
{
	...
	RegSetValueEx(key, _TEXT("SpellAutoFix"), 0, REG_DWORD, (LPBYTE)&m_spellAutoFix, sizeof(DWORD));
...
int CUserPref::getValue()
{
	...
	err = err || (ERROR_SUCCESS !=
					RegQueryValueEx(key, _TEXT("SpellAutoFix"), 0, &valType, (LPBYTE)&m_spellAutoFix, &nbytes));
