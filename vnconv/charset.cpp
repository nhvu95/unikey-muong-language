...
VnCharset * CVnCharsetLib::getVnCharset(int charsetIdx)
{
	...
	case CONV_CHARSET_UNI_MUONG:
		if (m_pUniMuongCharset == NULL)
			m_pUniMuongCharset = new UnicodeMuongCharset(UnicodeTable, UnicodeComposite);
		return m_pUniMuongCharset;
...
UnicodeMuongCharset::UnicodeMuongCharset(UnicodeChar * uniChars, DWORD * uniCompChars)
{
	m_uniMuongChars = uniCompChars;
	m_totalChars = 0;
	for (int i = 0; i<TOTAL_VNCHARS; i++) {
		m_info[i].compChar = uniCompChars[i];
		m_info[i].stdIndex = i;
...
int UnicodeMuongCharset::nextInput(ByteInStream & is, StdVnChar & stdChar, int & bytesRead)
{
	// read first char
...
int UnicodeMuongCharset::putChar(ByteOutStream & os, StdVnChar stdChar, int & outLen)
{
	int ret;
	if (stdChar >= VnStdCharOffset) {
		DWORD uniCompCh = m_uniMuongChars[stdChar - VnStdCharOffset];
		WORD lo = LOWORD(uniCompCh);
		WORD hi = HIWORD(uniCompCh);
		outLen = 2;
		ret = os.putW(lo);
