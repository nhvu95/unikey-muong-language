...
class UnicodeMuongCharset : public VnCharset {
protected:
	UniCompCharInfo m_info[TOTAL_VNCHARS * 2];
	DWORD *m_uniMuongChars;
	int m_totalChars;
public:
	UnicodeMuongCharset(UnicodeChar *uniChars, DWORD *uniCompChars);
	virtual int nextInput(ByteInStream & is, StdVnChar & stdChar, int & bytesRead);
	virtual int putChar(ByteOutStream & os, StdVnChar stdChar, int & outLen);
};
...
protected:
	...
	UnicodeMuongCharset *m_pUniMuongCharset;
