...
void PushBuffer(HWND wndFocused)
{
	...
	if ((pShMem->keyMode == UNICODE_CHARSET ) && pShMem->codeTable.encoding == UNICODE_UCS2) {
		if (!pShMem->options.useUnicodeClipboard && pShMem->unicodePlatform && 
			...
			HGLOBAL hBuf = GlobalAlloc(GMEM_MOVEABLE, sizeof(WORD) * VnKbd.keysPushed);// Cap phat vung nho cho 1 tien trinh
			HGLOBAL hBufAnsi = GlobalAlloc(GMEM_MOVEABLE, VnKbd.keysPushed);// Cap phat vung nho cho 1 tien trinh

			LPVOID pBuf = GlobalLock(hBuf);// tra ve con tro dau tien trong vung nho
