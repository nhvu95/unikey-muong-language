...
private:
	...
	void putBreveMarkForMuong(unsigned char c);
	void duplicateVowelForMuong(unsigned char c);
	void doubleChar(unsigned char c);
	void doubleCharForMuong(unsigned char c);
	void putToneMark(unsigned char c);
	void putToneMarkForMuong(unsigned char c);
	void shortKey(unsigned char c);
	void shortKeyForMuong(unsigned char c);
	void vniDoubleCharMark(unsigned char c);
	void throwBuf();  // used when buffer is full
	void autoFixSpell(unsigned char c); // only work for muong
	// helper functions
	void postProcess();
