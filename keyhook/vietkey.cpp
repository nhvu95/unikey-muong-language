...
WORD UnicodeMuongTone[] = { 0x301, 0x312, 0x309, 0x303, 0x32D };
//-----------------------------------------------
...
void VietKey::deleteCharBehind(int i, int &countDelete)
{
...
void VietKey::process(unsigned char c)
{
	...
	case BREVE_MARK:
		...
		else if (pShMem->inMethod == TELEX_MUONG && !lastWConverted && (c == 'w' || c == 'W'))
		{
			shortKeyForMuong(c);
			thisWConverted = 1;
		}
		else {
			if (pShMem->inMethod == TELEX_MUONG)
			{
				putBreveMarkForMuong(c);
				if (keysPushed == 0 && backs == 0 && (c == 'w' || c == 'W'))
				{
					shortKeyForMuong(c); // Special case for W key in TELEX_MUONG mode.
					thisWConverted = 1;
				}
			}
			else
	...
	case DOUBLE_KEY:
		...
		else if (pShMem->inMethod == TELEX_MUONG)
		{
			doubleCharForMuong(c);
		}
		break;
	case TONE_MARK:
		if (pShMem->inMethod == TELEX_INPUT)
		...
		{
			putToneMarkForMuong(c);
		}
		break;
	case SHORT_KEY:
		if (pShMem->inMethod == TELEX_INPUT) shortKey(c);
		else if (pShMem->inMethod == TELEX_MUONG) shortKeyForMuong(c);
		break;
	...
	if (keysPushed == 0 && backs == 0) {
		if (pShMem->inMethod != VIQR_INPUT &&
			...
			return;
		if (pShMem->options.spellAutoFix && pShMem->inMethod == TELEX_MUONG && c == ' ')
		{
			autoFixSpell(c);
			//	backs = 0;
				//keys = 0;
...
void VietKey::postProcess()
{
	...
	if (pShMem->keyMode == DECOMPOSED_UNICODE_CHARSET || pShMem->keyMode == MUONG_CHARSET) {
		WORD base, tone;
		for (i = k = 0, j = keys - keysPushed; i < keysPushed; i++, j++) {
			...
			tone = HIBYTE(mapping);
			if (tone > 0) {
				switch (pShMem->keyMode)
				{
				case MUONG_CHARSET:
					uniPush[k++] = UnicodeMuongTone[tone - 1];
					break;
				default:
...
int VietKey::keyCategory(unsigned char c)
{
	...
	if ((pShMem->inMethod == TELEX_INPUT || pShMem->inMethod == TELEX_MUONG) && index > 0 && index < 9)
		return DOUBLE_KEY;
	if (pShMem->inMethod != TELEX_INPUT && ATTR_VNI_DOUBLE_INDEX(attr) > 0)
		return VNI_DOUBLE_CHAR_MARK;

	if (pShMem->inMethod == TELEX_MUONG && ATTR_IS_DUPLICATE(attr) > 0)
		return DUPLICATE_MARK;
...
void VietKey::putBreveMarkForMuong(unsigned char c)
{
	int i, k, saveI = NULL_VALUE;
	long attr;
	unsigned char t, newChar;
	int leftMost;
	...
	if (pShMem->options.freeMarking && i > 0)
	{
		unsigned char prevChar;
		int tmpIdx;
		if (ATTR_VOWEL_INDEX(DT[prevChar = buf[i - 1]]) > 0)// if prev char is vowel
			prevChar = BD[ATTR_VOWEL_INDEX(DT[prevChar]) - 1][5];
		...
		if (tmpIdx > 4)
			prevChar = WReverse[tmpIdx - 4 - 1];
		if ((prevChar == 'o' || prevChar == 'O' || prevChar == 'u' || prevChar == 'U') &&
			(buf[i] == 'u' || buf[i] == 'U' || buf[i] == 'o' || buf[i] == 'O')) {
			saveI = i;
			i--;
			toneIndex = ATTR_CURRENT_TONE(DT[buf[i]]);
		}
		...
		if (i > 0)
		{
			...
			/*	if ((prevChar == 'U' || prevChar == 'u') &&
					(i == 1 || (i > 1 && buf[i - 2] != 'q' && buf[i - 2] != 'Q')))
				{
					...
					tmpIdx = ATTR_DBCHAR_INDEX(DT[t]) - 4;
					t = toupper(t);
					if ((t == 'A' && pShMem->inMethod == TELEX_MUONG) ||
						((t == 'O' || WReverse[tmpIdx - 1] == 'o' || WReverse[tmpIdx - 1] == 'O') && i != keys - 1))
					{
						i--;
						toneIndex= ATTR_CURRENT_TONE(DT[buf[i]]);
	...
	} while (saveI != NULL_VALUE);
	for (k = i + 1; k < keys; k++)
		ansiPush[keysPushed++] = buf[k];
}

void VietKey::duplicateVowelForMuong(unsigned char c)
{
	int i, k, saveI = NULL_VALUE;
	long attr;
	unsigned char t, newChar, prevChar;
	int leftMost;
	...
	while (i >= leftMost) {
		...
		toneIndex = ATTR_CURRENT_TONE(DT[buf[i]]);
		if (toneIndex == 0 || toneIndex == 6)
			index = ATTR_VOWEL_INDEX(attr);//here		
		else
		{
			index = ATTR_DBCHAR_INDEX(DT[BD[ATTR_VOWEL_INDEX(attr) - 1][5]]);
		}
		if (index > 0) { // from 11 are the indexes of VNI double-charater marks
			if (pShMem->inMethod != TELEX_MUONG) {
				if ((index_c == VNI_HORN_INDEX && index > 6) ||
					(index_c == VNI_BREVE_INDEX && index <= 6))
					break;
	...
	if (pShMem->options.freeMarking && i > 0)
	{
		...
		if (ATTR_VOWEL_INDEX(DT[prevChar = buf[i - 1]]) > 0)// if prev char is vowel
		{
			...
			{
				//backs = keys - i;
				/*ansiPush[keysPushed++] = i + '0';
				ansiPush[keysPushed++] = keys + '0';*/
				for (int k = i; k < keys; k++)
					buf[k] = buf[k + 1];
				keys -= 1;
				//keys -= 2;
				backs = keys - i;
...
void VietKey::doubleCharForMuong(unsigned char c)
{
	int i, k, leftMost;
	long attr;
	unsigned char newChar;
	...
	// if oeo, o must not be understood as a double character
	if (pShMem->inMethod == TELEX_MUONG && toupper(c) == 'O' && i < keys - 1) {
		unsigned char ch = buf[i + 1];
		int vowelIndex = ATTR_VOWEL_INDEX(DT[ch]);
		if (vowelIndex > 0 && BD[vowelIndex - 1][5] == 'e')
			return;
...
void VietKey::shortKeyForMuong(unsigned char c)
{
	...
	if ((keysPushed == 0 || backs == 0) && !shortchar)
		ansiPush[keysPushed++] = c;
	else ansiPush[keysPushed++] = newChar;
...
void VietKey::putToneMarkForMuong(unsigned char c)
{
	int i, k, l, cuoi, index, vowel, duplicate, leftMost;
	unsigned char newChar, t;
	long attr;
...
void VietKey::throwBuf()
{
	...
	keys = KEYS_MAINTAIN;
	lastKeys = LAST_KEYS_MAINTAIN;
}

void VietKey::autoFixSpell(unsigned char c)
{
	int i = lastKeys, countDelete = 0;
	unsigned char tmpOldChar;
	while (i < keys) {
		tmpOldChar = buf[i];
		switch (buf[i])
		{
		...
		case 'q':
			if (i == lastKeys)
			{
				if (buf[i] == 'c' && buf[i + 1] != 'h') buf[i] = (unsigned char)'k';
				if (buf[i] == 'q') buf[i] = (unsigned char)'k';
				if (buf[i + 1] == 'u' && (int)buf[i + 2] != 232) buf[i + 1] = (unsigned char)'w'; //232 la chu o^ co dau sac
			}
			break;
		case 't':
			if (i == lastKeys)
		...
		case 'g':
			...
			{
				// Neu la 'gi'thi xoa 'i'
				if (buf[i] == 'g' && buf[i + 1] == 'i')
				{
					deleteCharBehind(i, countDelete);
					break;
				}
				// Neu sau 'g' co 'h' thi xoa 'h' 
				...
				// Neu la 'd, gi, g' dung dau tu thi chuyen thanh 'z';
				if (buf[i] == 'd' || buf[i] == 'g')
					buf[i] = (unsigned char)'z';
			}

			break;
			//Neu la 's' thi chuyen thanh 'x'
		case 's':
			if (i == lastKeys)
			{
				if (buf[i] == 's') buf[i] = (unsigned char)'x';
			}
			break;
			//Neu da co 'w' thi se khong co 'u' 'o' sau no
		case 'w':
			if (buf[i + 1] == 'o' || buf[i + 1] == 'u')
			{
				deleteCharBehind(i, countDelete);
			}
			break;
		case 'u':
			...
			break;
			//đối với kh thì thay o, u bằng w, trừ chữ khua
		case 'k':
			if (i == lastKeys)
			{
				if (buf[i + 1] == 'h')
				{
					if ((buf[i + 2] == 'o' || buf[i + 2] == 'u') && (buf[i + 3] != 'a'))
					{
						buf[i + 2] = 'w';
					}
				}
		...
		lowerCase[keys] = tmpOldChar;
		i++;
	}
	//(OldChar);
	backs = keys - lastKeys + countDelete;
	for (int k = lastKeys; k < keys; k++)
		ansiPush[keysPushed++] = buf[k];
	//ChangedChar = buf[i];
	putChar(c, islower(OldChar));
	ansiPush[keysPushed++] = c;
...
int VietKey::checkMacro(unsigned char lastChar)
{
	...
	else if (pShMem->keyMode == DECOMPOSED_UNICODE_CHARSET || pShMem->keyMode == MUONG_CHARSET) {
		WORD *p = (WORD *)(pShMem->macroMem + pMacro->textOffset);
		for (keysPushed = 0; p[keysPushed]; keysPushed++)
			uniPush[keysPushed] = p[keysPushed];
		uniPush[keysPushed++] = lastChar;
